...
int     main (int ac, const char **av)
{
    ...
    std::map<std::string, ICracker *> Crackers;
    std::map<std::string, IBrute *> Brutus;
    ...
    Crackers.insert(std::make_pair("zip", new zipCracker()));
    Brutus.insert(std::make_pair("random", new randomBrute()));
    ...
    Crackers[crack]->configure(options);
    Brutus[mode]->configure(options);
    Brutus[mode]->setCracker(Crackers[crack]);

    //Let's gooooo
    Brutus[mode]->brute();

    for (auto cracker: Crackers){ delete cracker.second; }
    for (auto tus: Brutus){ delete tus.second; }
    delete args;
    return 0;
}
